...
-- Execution and evaluation status separation
alter table botops_runs add column if not exists execution_status text not null default 'running';
alter table botops_runs add column if not exists evaluation_status text not null default 'pending';
alter table botops_runs add column if not exists completion_client_id text;

-- Add CHECK constraints for execution_status and evaluation_status
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_execution_status') THEN
    ALTER TABLE botops_runs ADD CONSTRAINT chk_execution_status
      CHECK (execution_status IN ('running', 'completed', 'failed', 'stalled'));
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_evaluation_status') THEN
    ALTER TABLE botops_runs ADD CONSTRAINT chk_evaluation_status
...
create unique index if not exists idx_botops_runs_completion_client
  on botops_runs(completion_client_id)
  where completion_client_id is not null;

-- Evaluator result uniqueness (prevent duplicates on retry)
create unique index if not exists idx_botops_eval_results_run_eval_version
  on botops_evaluator_results(bot_run_id, evaluator_name, evaluator_version);

-- Field comparison uniqueness (prevent duplicates on retry)
create unique index if not exists idx_botops_field_comp_run_field
  on botops_field_comparisons(bot_run_id, field_name);
...
create index if not exists idx_botops_runs_execution_status on botops_runs(execution_status);
create index if not exists idx_botops_runs_evaluation_status on botops_runs(evaluation_status);
...
-- BotOps RLS — permissive for demo (idempotent DO blocks)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_runs_all') THEN
    create policy "botops_runs_all" on botops_runs for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_events_all') THEN
    create policy "botops_events_all" on botops_events for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_evaluator_results_all') THEN
    create policy "botops_evaluator_results_all" on botops_evaluator_results for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_field_comparisons_all') THEN
    create policy "botops_field_comparisons_all" on botops_field_comparisons for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_qa_review_actions_all') THEN
    create policy "botops_qa_review_actions_all" on botops_qa_review_actions for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_audit_logs_all') THEN
    create policy "botops_audit_logs_all" on botops_audit_logs for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_summaries_all') THEN
    create policy "botops_summaries_all" on botops_summaries for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_regression_baselines_all') THEN
    create policy "botops_regression_baselines_all" on botops_regression_baselines for all using (true) with check (true); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'botops_artifacts_all') THEN
    create policy "botops_artifacts_all" on botops_artifacts for all using (true) with check (true); END IF;
END $$;
